...
func (s *session) get(rest []string) {
	...
	defer f.Close()
	if err := s.d.Download(s.ctx, s.g, remote, f); err != nil {
		fmt.Fprintf(os.Stderr, "get: %v\n", err)
		return
	}
	fmt.Printf("got /%s -> %s\n", remote, localPath)
...
func (s *session) put(rest []string) {
	...
	if err := s.d.Upload(s.ctx, s.g, remote, ctype, data); err != nil {
		fmt.Fprintf(os.Stderr, "put: %v\n", err)
		return
	}
	fmt.Printf("put %s -> /%s (%d bytes)\n", localPath, remote, len(data))
}
...
func printHelp() {
  ...
  put <local> [remote] upload a file (<=250MB)
